...
export default function TaskModal({ open, task, defaults, onClose, onSaved }) {
  ...
  // image attachment
  const [existingImage, setExistingImage] = useState(null) // path saved on task (edit mode)
  const [imageFile,     setImageFile]     = useState(null) // newly selected file (pending upload)
  const [imagePreview,  setImagePreview]  = useState(null) // object URL for preview
  const [removeImage,   setRemoveImage]   = useState(false) // edit mode: clear existing on save
  ...
  // Active card lookup for the selected box (null = none, undefined = not yet loaded)
  const [activeCard, setActiveCard] = useState(undefined)
  const [loadingCard, setLoadingCard] = useState(false)

  // Load lookups whenever the modal opens (cheap, cached by browser anyway)
  useEffect(() => {
    if (!open) return
    let cancelled = false
    Promise.all([
  ...
  // Revoke object URL when preview changes / unmounts to avoid leaks
  useEffect(() => {
  ...
  // A task type that does NOT open a card (i.e. removal / transfer-close /
  // any non-lifecycle work like reporting/maintenance) attaches to the
  // existing active card. Without one, creation must be blocked.
  const requiresActiveCard = !!selectedType && !selectedType.opens_card

  // Fetch the active card whenever the selected box changes (create mode only).
  useEffect(() => {
  ...
  // Collection-task hint: this task type grants the assigned collector
  // temporary access to the box even if it is outside their area rules.
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      let saved
      if (isEdit) {
        // Backend PUT /tasks/:id ignores task_type_id/box_id — they can't be changed.
        // Send only the editable fields. image_path: include only when changed.
        const patch = {
