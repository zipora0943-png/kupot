...
/**
 * Confirm-execution modal for a task.
 *
 * Behavior depends on the task's task_type flags (returned from backend):
 *   opens_card  &&  closes_card → relocation (close + open new)
 *   opens_card  && !closes_card → installation (open new card)
 *  !opens_card  &&  closes_card → removal (close active card)
 *  !opens_card  && !closes_card → generic (just creates a TASK_DONE event)
 *
 * For tasks that open a card the user MUST enter at least a city.
 * Pre-fills the location fields from `task.new_*` if set when the task was created.
 *
 * Props:
 *   open      — boolean
 *   task      — full task row (must include opens_card, closes_card, type_name, icon, iron_number)
 *   onClose   — () => void
...
export default function TaskExecModal({ open, task, onClose, onSuccess, onReportNotExecuted }) {
  ...
  // Whenever a different task is opened, reset / pre-fill from task.new_*
  useEffect(() => {
    if (!open || !task) return
  ...
  // Task 48: load box types only when the user needs to enter a box now.
  useEffect(() => {
